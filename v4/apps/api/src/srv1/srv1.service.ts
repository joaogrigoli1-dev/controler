...
import type { HostMetrics, ContainerSummary } from "@controler/shared";
...
export class Srv1Service {
  ...
  async getHostMetrics(): Promise<HostMetrics> {
    return this.redis.cached("srv1:host-metrics", 30, async () => {
      ...
      const cpuPercent = metrics ? (lastVal(metrics.cpu_usage) ?? 0) : 0;
      const memUsedBytes = metrics ? (lastVal(metrics.ram_usage) ?? 0) : 0;
      const diskUsedBytes = metrics ? (lastVal(metrics.disk_space) ?? 0) : 0;
      const netIn = metrics ? (lastVal(metrics.incoming_traffic) ?? 0) : 0;
      const netOut = metrics ? (lastVal(metrics.outgoing_traffic) ?? 0) : 0;
      const uptimeSeconds = metrics ? (lastVal(metrics.uptime) ?? 0) : 0;
