    ...
    <div style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 6, right: 8, bottom: 0, left: -14 }}>
          ...
          {typeof warn === "number" && (
            <ReferenceLine y={warn} stroke="hsl(45 95% 53%)" strokeDasharray="4 4" strokeOpacity={0.6} />
          )}
          {typeof crit === "number" && (
            <ReferenceLine y={crit} stroke="hsl(0 90% 71%)" strokeDasharray="4 4" strokeOpacity={0.7} />
          )}
          {series.map(s => (
            <Line
              key={s.key}
